...
for num, APmodel_name in enumerate(model_list):
    ...
    if APmodel_name == 'ORd-CiPA':
        ...
        scaling_df = pd.read_csv(scaling_df_filepath, index_col=[0])
        scaling_factor = scaling_df.loc['hERG_peak']['conductance scale']
        print(APmodel_name, ': ', scaling_factor)
        log = AP_model.model_simulation(repeats,
                                        conductance_name='tune.ikr_rescale',
                                        conductance_value=scaling_factor,
                                        abs_tol=abs_tol, rel_tol=rel_tol)
    ...
    for i in range(4):
        if i == num:
            r, c = int(num / 2), num % 2
            fig.axs[r][c].plot(log.time(), log[IKr_key], 'k', zorder=5)
            fig.axs[r][c].set_title(model_label[num])
        else:
            r, c = int(i / 2), i % 2
            fig.axs[r][c].plot(log.time(), log[IKr_key], '#cccccc', alpha=0.9)
...
fig.savefig(fig_dir + 'IKr_magnitude.svg')
