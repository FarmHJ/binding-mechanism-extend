...
axs.set_xlim(min(Vhalf_range), max(Vhalf_range))
axs.set_ylim(min(np.log10(Kmax_range)), max(np.log10(Kmax_range)))
# axs[i].set_zlim(min(np.log10(Ku_curve)), max(np.log10(Ku_curve)))
axs.set_zlim(min(np.log10(Ku_range)), max(np.log10(Ku_range)))

axs.xaxis.set_major_locator(mticker.MaxNLocator(nbins=6))
axs.yaxis.set_major_formatter(mticker.FuncFormatter(log_tick_formatter))
axs.yaxis.set_major_locator(mticker.MaxNLocator(integer=True))
