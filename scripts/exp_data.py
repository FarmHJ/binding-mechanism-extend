...
fig_protocol = modelling.figures.FigureStructure(
    figsize=(8, 2), gridspec=(1, len(protocol_list)), wspace=0.25)
...
for drug_count, drug in enumerate(drug_list):
    for protocol_count, protocol in enumerate(protocol_list):
        ...
        # for cell in [cell_list['cells'].values[0]]:
            ...
            for sweep in range(total_pulses):
                ...
                if compound == previous_compound:
                    ...
                    compound_count += 1
                    # print('Change of compound:', sweep)

            axs[drug_conc_index][cell_num_index].text(1, 0, cell, fontsize=6,
                                                      ha='right', va='bottom',
                                                      color='k')
        ...
        filename = protocol + "_" + drug + "2.pdf"
        fig.savefig("../figures/experimental_data/" + filename)

#         panel_protocol = fig_protocol.axs[0][protocol_count]
#         panel_protocol.plot(time, stimulus, 'k', zorder=5)
#         panel_protocol.set_ylabel('Voltage (mV)')
#         panel_protocol.set_xlabel('Time (ms)')
#         panel_protocol.set_ylim(-90, 50)
#         panel_protocol.set_xlim(0, max(time))
#         panel_protocol.grid(linestyle='--')

# ramp_prot_ax = fig_protocol.axs[0][0]
# x_ticks = ramp_prot_ax.get_xticks()
# x_ticks = np.concatenate((x_ticks, np.array([550, 646])))
# ramp_prot_ax.set_xticks(x_ticks)
# x_tick_labels = ramp_prot_ax.get_xticklabels()
# x_tick_labels[-1].set_rotation(45)
# # x_tick_labels[-1].set_ha('right')
# x_tick_labels[-2].set_rotation(45)

# ramp_prot_ax.set_title('Ramp protocol')
# fig_protocol.axs[0][1].set_title('Step protocol')
