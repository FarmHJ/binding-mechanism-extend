...
# concentration.
# One is by dD/dt = 0 and D_0 = d, while the other is D = d.
import myokit
import numpy as np
import os
...
class ModelSimController(object):
    ...
    def __init__(self, model_name, ikr_modified=True, cycle_length=1000,
        ...
        if model_name in ['Lei', 'Li']:
            self._cycle_length = 25e3
            protocol = modelling.SD_details.Milnes(self._cycle_length)
        ...
        self.current_keys = model_current_keys[self.model_name]
    ...
    def set_ikr_rescale_method(self, tuning_method):
        ikr_rescale = modelling.model_naming.load_ikr_scaling(
            self.model_name, tuning_method)
        self.set_ikr_rescale(ikr_rescale)
    ...
    def extract_peak(self, signal_log, current_name=None,
                     peak_reduction_output=False):

        if current_name is None:
            current_name = self.ikr_key
        pulses = len(signal_log.keys_like(current_name))
        current_key_list = [x for x in signal_log.keys()
                            if x.endswith(current_name)]
        current_key_list.sort()
        pulses = len(current_key_list)

        peaks = []
        for i in range(pulses):
            peaks.append(np.max(signal_log[current_key_list[i]]))

        if peak_reduction_output:
            if peaks[0] == 0:
                peak_reduction = 0
            else:
