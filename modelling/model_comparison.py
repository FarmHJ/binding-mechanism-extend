...
import modelling
# TODO: raise warning if Hill curve not formed
...
class ModelComparison(object):
    ...
    def __init__(self, APsimulator, IKr_simulator=None):
        ...
        if IKr_simulator is not None and not \
            ...
            raise TypeError(
                'The model has to be an instance of a ''Lei'
                'modelling.ModelSimController.')
    ...
    def prepare_inputs(self, input, skip_ikr=False):
        ...
        self.param_df = param_values

        if skip_ikr:
            drug_conc = input['drug_conc_Hill']
            self.drug_conc_ikr = np.array(drug_conc)[~np.isnan(drug_conc)]
            self.Hill_coef = [input['Hill_curve']['Hill_coef'],
                              input['Hill_curve']['IC50']]
    ...
    def _get_apd90(self, drug_conc, save_signal=2):
        ...
        plt.figure()
        plt.plot(log.time(), log[self.APsim.Vm_key, 0], 'k-', label='SD')
        plt.plot(np.array(log.time()) + self.APsim._cycle_length,
                 log[self.APsim.Vm_key, 1], 'k-')

        apd90_SD = self.APsim.APD90(log)
        # print('Compute APD90 for SD model: ', time.time() - start)
    ...
    def get_APD(self, drug_conc=None, save_signal=2, data_points=20, EAD=True):
        ...
        if EAD:
            ...
            checker_count = max(sum(checker_trapping),
                                sum(checker_conductance))
            counter = 0
            while checker_count < 1 and counter < 10:
                drug_conc = drug_conc + [max(drug_conc) * np.sqrt(10)]
    ...
    def ME_ratio(self):
        ...
        for i in range(len(self.APD_trapping)):
            if not np.isnan(self.APD_trapping[i]) and not \
                    np.isnan(self.APD_conductance[i]):
                ratio_sum += (self.APD_trapping[i] - self.APD_conductance[i]) \
                    / self.APD_conductance[i]
                count += 1

        if count != 0:
            self.Error = ratio_sum / count
        else:
    ...
    def RMSE_ratio(self):
        ...
        for i in range(len(self.APD_trapping)):
            if not np.isnan(self.APD_trapping[i]) and not \
                    np.isnan(self.APD_conductance[i]):
                ratio_sum += (self.APD_trapping[i] - self.APD_conductance[i])**2 \
                    / self.APD_conductance[i]**2
                count += 1
    ...
    def process_data(self, save_orig_halfmax=False):
        ...
        conc_ikr_ind = [f'conc_{i}' for i in range(len(self.drug_conc_ikr))]
        conc_AP_ind = [f'conc_{i}' for i in range(len(self.drug_conc_AP))]
        ...
        data_list = list(self.drug_conc_ikr) + list(self.Hill_coef) + \
            list(self.param_df.values()) + list(self.drug_conc_AP) + \
